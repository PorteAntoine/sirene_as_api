class CreateEtablissementMayToJunes < ActiveRecord::Migration[5.0]
  def change
    create_table :etablissement_may_to_junes do |t|
    ...
    execute "
      create index on etablissement_may_to_junes using gin(to_tsvector('french', siren));
      create index on etablissement_may_to_junes using gin(to_tsvector('french', siret));
      create index on etablissement_may_to_junes using gin(to_tsvector('french', activite_principale));
      create index on etablissement_may_to_junes using gin(to_tsvector('french', l6_normalisee));
      create index on etablissement_may_to_junes using gin(to_tsvector('french', nom_raison_sociale));"
  end
end
